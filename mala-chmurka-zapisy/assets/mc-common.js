...
/* Linki konta. Podmieniamy je po rozstrzygnięciu logowania — patrz updateNavAuth(). */
const NAV_AUTH = [
  ['Zaloguj się',    URLS.login],
  ['Zarejestruj się', URLS.login + '?tab=register']
];
    ...
    <div class="container header-inner">
      ...
      <nav class="nav" id="mcNav" aria-label="Menu główne">
        <ul>
          ...
          <li class="nav-auth" id="mcNavAuth">${NAV_AUTH.map(([label, href]) =>
            `<a href="${href}">${label}</a>`).join('')}</li>
    ...
    <div class="container footer-inner">
      ...
      <nav class="footer-nav" aria-label="Menu w stopce">
        ...
        <ul>
          ...
          <li><a href="${URLS.login}">Zaloguj się</a></li>
        </ul>
      </nav>
...
export function updateNavAuth(user) {
  ...
  if (!box) return;
  if (!user) {
    box.innerHTML = NAV_AUTH.map(([label, href]) => `<a href="${href}">${label}</a>`).join('');
    return;
  }
  const name = user.displayName || (user.email || '').split('@')[0] || 'Moje konto';
  box.innerHTML = `<a href="${URLS.history}" title="${esc(user.email || '')}">${esc(name)}</a>` +
                  `<a href="#" id="mcNavOut">Wyloguj</a>`;
  const out = document.getElementById('mcNavOut');
